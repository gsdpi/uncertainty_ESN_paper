##################################################################
# Main script for ICANN dataset processing with MiniRocket
...
def process_signal_minirocket(df, features, signal_label, show_roc_plot=False):
    """
    Process a single signal using MiniRocket anomaly detection.
    ...
    if show_roc_plot:
        ...
        plt.title(f'ROC Curve - {signal_label} (MiniRocket)')
...
def process_all_signals_minirocket(df):
    """
    Process different signal combinations using MiniRocket anomaly detection.
    ...
    for features, signal_label in signal_configs:
        results = process_signal_minirocket(
            df, features,
...
if __name__ == '__main__':
    print('='*70)
    print('ICANN MiniRocket Anomaly Detection Pipeline')
    print('='*70)
    print(f'\nParameters:')
    ...
    results = process_all_signals_minirocket(df)
